...
### `calculate_main_stats.py`

Creates the compact workbook used for the main dissertation statistics tables
from the larger `review_honeypot_comparison.xlsx` workbook generated by
`compare_honeypots.py`. The script reads the `Summary` and `MatchedCohort`
sheets, verifies the values used in the compact tables, and writes a chart-free
workbook with the same table layout as `HoneypotAnalysisGraphics - Kopie.xlsx`.

Run it after recreating the large comparison workbook:

```bash
python calculate_main_stats.py review_honeypot_comparison.xlsx -o "HoneypotAnalysisGraphics - Kopie.xlsx"
```

If `-o` is omitted, the output is written to
`HoneypotAnalysisMainStats.xlsx` next to the input workbook. The login success
and acceptance-rate rows use the per-IP mean acceptance metric: for each source
IP, `login_ok_sessions / sessions` is calculated first, then those per-IP rates
are averaged over the relevant IP set.
